create or replace function fn_get_group_notification_summary(
  p_group_id uuid,
...
with params as (
  select
    greatest(coalesce(p_limit, 11), 1) as result_limit,
    greatest(coalesce(p_limit, 11), 1) * 4 as source_limit,
...
select jsonb_build_object(
  'unread_count',
  count(*) filter (where read_rows.notification_id is null)
)
from ranked
left join read_rows
