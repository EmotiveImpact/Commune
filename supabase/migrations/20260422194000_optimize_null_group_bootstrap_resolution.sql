...
group_summaries as (
  select
    ...
    row_number() over (order by lower(g.name) asc, g.id asc) as sort_order
  from memberships
...
resolved_group as (
  select coalesce(
    (
      ...
      where group_summaries.id = p_active_group_id
      limit 1
    ),
    (
      select group_summaries.id
      from group_summaries
      where group_summaries.sort_order = 1
      limit 1
    )
  ) as group_id
)
select jsonb_build_object(
  ...
  coalesce(
    (
      select jsonb_agg((to_jsonb(group_summaries) - 'sort_order') order by group_summaries.sort_order)
      from group_summaries
    ),
    '[]'::jsonb
  ),
