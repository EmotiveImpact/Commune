  ...
  if (overdue.length > 0) {
    ...
    for (const [groupId, count] of groupedByGroup) {
      nudges.push({
        ...
        type: 'unpaid_expense',
        priority: 1,
        title: `${count} overdue expense${count > 1 ? 's' : ''}`,
        description: `${count} expense${count > 1 ? 's' : ''} past due date in ${groupNameMap.get(groupId) ?? 'a group'}.`,
  ...
  for (const settlement of input.settlements) {
    ...
    if (othersOweYou.length > 0) {
      nudges.push({
        ...
        type: 'others_pending',
        priority: 2,
        title: `${othersOweYou.length} people owe you`,
        description: `In ${settlement.groupName}, ${othersOweYou.length} member${othersOweYou.length > 1 ? 's' : ''} still owe you money.`,
